...
void physics_collision_handler(body_t *body1, body_t *body2, vector_t axis,
  ...
  // Projecting the velocity onto the axis is essentially what u_a and u_b is
  // hence dot product between velocity and axis of importance
  double u_a = vec_dot(vel1, axis);
  double u_b = vec_dot(vel2, axis);

  double numerator = -(1 + force_const) * (u_a - u_b);
  ...
  double impulse_mag = reduced_mass * numerator;
  ...
  if (mass2 != INFINITY) {
    // Opposite to the way body 1 is moving
