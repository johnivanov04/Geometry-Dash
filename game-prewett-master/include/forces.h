...
/**
 * Frees the memory allocated for a forcer
 *
 * @param force the forcer to free
 */
void forcer_free(forcer_t *force);

/**
 * Initializes a new force creator structure, which is used to apply forces
 * between bodies. The created forcer will be responsible for the specified
 * force creation function
 *
 * @param creator the function that creates and applies forces between the
 * bodies
 * @param aux auxiliary data that will be passed to the creator function
 * @param bodies a list of bodies that the force applies to
 * @return a pointer to the newly initialized forcer
 */
