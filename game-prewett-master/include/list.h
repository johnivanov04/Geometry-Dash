...
/**
 * Sets the element at a given index in the list to a new value. If the index is
 * already occupied, the existing value is overwritten.
 *
 * @param list a pointer to a list returned from list_init()
 * @param index the index at which to set the new value, must be within the
 *        current size of the list (not the capacity)
 * @param value the new value to set at the specified index, must not be NULL
 */
void list_set(list_t *list, size_t index, void *value);
